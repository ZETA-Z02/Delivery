...
function mostrarPedidos() {
  ...
  $.ajax({
    url: "http://localhost/delivery/controller/ZetaControllerMaster.php?actionPersonalView=mostrarPedidosPendientes",
    type: "GET",
    success: function (response) {
      ...
      pedidosAll.forEach((pedido) => {
        templatePedidos += `
            <details>
              <summary>
                  <span class="faq-title">
                    <h3>${pedido.id_pedido}-|-${pedido.id_cliente}-|-${pedido.destino}-|-id personal encargado:${pedido.id_personal}|ESTADO: ${pedido.estado}</h3>
                    <button class="btn-completar" id=${pedido.id_pedido}>Marcar como Entregado</button>
                  </span>
            ...
            `;
        $.ajax({
          url: "http://localhost/delivery/controller/ZetaControllerMaster.php?actionPersonalView=mostrarDetalles",
...
// Agregar un manejador de eventos para los botones "Marcar como Entregado"
$(".faq-container").on("click", ".btn-completar", function () {
  const idPedido = $(this).attr("id");

  //console.log("ID del pedido marcado como entregado:", idPedido);

  $.post(
    "http://localhost/delivery/controller/ZetaControllerMaster.php?actionPersonalView=actualizarEstado",
    { id_pedido: idPedido, estado: "completado" },
    function (response) {
      console.log(response);
    }
  );
